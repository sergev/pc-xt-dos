...
{
#else
int _flushbu(int c,FILE *fpf)
{
	FILE _near *fp = (FILE _near *)fpf;
#endif
...
#endif
	fp->_ptr++;
	fp->_cnt = _bufsize(fp) - 1; /* # of chars left in buffer	*/
	return c & 0xFF;
...
{
#else
int _fillbuf(FILE *fpf)
{
	FILE _near *fp = (FILE _near *)fpf;
#endif

#if _WINDOWS
/*	sleep(0); Messes up WINC < yield to other Windows programs> */
#endif
